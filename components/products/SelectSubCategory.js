...
const SelectSubCategory = ({ label, items, selected, setSelected }) => {
    return (
            ...
            {({ open }) => (
                <>
                    ...
                    <div className="relative mt-1">
                        <Listbox.Button className="relative h-11 w-full cursor-pointer rounded-md border border-gray-300 bg-white py-2 pr-10 text-left focus:border-green-500 focus:outline-none focus:ring-1 focus:ring-green-500 sm:text-sm">
                            <span className="flex items-center font-medium text-gray-600">
                                <span className="ml-3 block truncate">{selected?.name || "Select Subcategory"}</span>
                            </span>
